...
function FormWrapper({ children, inputRef, show }: FormWrapperProps) {
  ...
  return (
    <Form
      action={goSearch}
      className={clsx(
        "w-full min-w-full px-2 hidden sm:flex h-[70px] bg-inherit  top-0 left-0  absolute items-center  sm:w-auto md:bg-transparent sm:h-auto  sm:static  sm:flex-none sm:items-start",
        {
          unhide: show,
        },
      )}
